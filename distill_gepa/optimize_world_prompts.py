...
async def evaluate_prompt_on_group_async(
    *,
    ...
    model_attempts: int,
    shuffle_namespace: str,
) -> tuple[dict[str, Any], list[QuestionPromptEvaluation]]:
    evaluations = list(
    ...
    return aggregate_group_metrics(evaluations), evaluations
...
def build_result_record(
    *,
    ...
    baseline_evaluations: Sequence[QuestionPromptEvaluation],
    optimized_evaluations: Sequence[QuestionPromptEvaluation],
    gepa_report: dict[str, Any],
) -> dict[str, Any]:
    baseline_by_id = {item.question.question_id: item for item in baseline_evaluations}
    question_deltas: list[dict[str, Any]] = []
    improved_question_count = 0
    for optimized in optimized_evaluations:
        baseline = baseline_by_id[optimized.question.question_id]
        improved = (not baseline.stable_correct) and optimized.stable_correct
        improved_to_distillable = improved and optimized.usable_for_distill_rate >= 1.0 and optimized.think_tag_rate >= 1.0
        improved_question_count += int(improved)
        question_deltas.append(
            {
                "question_id": optimized.question.question_id,
                "question": optimized.question.to_dict(),
    ...
    with asyncio.Runner() as event_loop_runner:
        ...
        baseline_metrics, baseline_evaluations = event_loop_runner.run(
            evaluate_prompt_on_group_async(
                ...
                model_attempts=model_attempts,
                shuffle_namespace=build_shuffle_key("gepa_materialize", target_model_name, group_id),
            )
        )
